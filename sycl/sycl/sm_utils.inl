...
#if DPCPP_COMPATIBILITY_TEMP >= 700
...
static __dpct_inline__ void atomic_add(double *address, double value)
{
  ...
  unsigned long long old = dpct::bit_cast<double, long long>(address[0]),
                     assumed;
  do { 
    assumed = old;
    old = dpct::atomic_compare_exchange_strong(
        ...
        (unsigned long long)(dpct::bit_cast<double, long long>(
            value + dpct::bit_cast<long long, double>(assumed))));
  } 
  while( assumed != old ); 
}
...
struct Ld<LD_AUTO> 
{
  template <typename T> static __dpct_inline__ T load(const T *ptr) { return *ptr; }
};

template<>
struct Ld<LD_CG> 
{
  static __dpct_inline__ int load(const int *ptr)
  { 
    int ret; 
  ...
  static __dpct_inline__ float load(const float *ptr)
  { 
    float ret; 
  ...
  static __dpct_inline__ double load(const double *ptr)
  { 
    double ret; 
...
struct Ld<LD_NC> 
{
  template <typename T> static __dpct_inline__ T load(const T *ptr) { return __ldg( ptr ); }
};
...
static __dpct_inline__ void load_vec2(float (&u)[2], const float *ptr)
{
  asm( "ld.global.cg.f32.v2 {%0, %1}, [%2];" : "=f"(u[0]), "=f"(u[1]) : __PTR(ptr) );
}

static __dpct_inline__ void load_vec2(double (&u)[2], const double *ptr)
{
  asm( "ld.global.cg.f64.v2 {%0, %1}, [%2];" : "=d"(u[0]), "=d"(u[1]) : __PTR(ptr) );
}

static __dpct_inline__ void load_vec4(float (&u)[4], const float *ptr)
{
  asm( "ld.global.cg.f32.v4 {%0, %1, %2, %3}, [%4];" : "=f"(u[0]), "=f"(u[1]), "=f"(u[2]), "=f"(u[3]) : __PTR(ptr) );
}

static __dpct_inline__ void load_vec4(double (&u)[4], const double *ptr)
{
...
#elif DPCPP_COMPATIBILITY_TEMP >= 300
...
struct Add
{
  ...
  static __dpct_inline__ Value_type eval(Value_type x, Value_type y) { return x+y; }
};

#if defined(DPCPP_COMPATIBILITY_TEMP) && DPCPP_COMPATIBILITY_TEMP >= 300
...
{
  template< typename Operator, typename Value_type >
  static __inline__ Value_type execute( volatile Value_type *smem, Value_type x ,
    ...
    for( int offset = WARP_SIZE / 2 ; offset >= NUM_THREADS_PER_ITEM ; offset >>= 1 )
      if( my_lane_id < offset )
        smem[item_ct1.get_local_id(2)] = x =
            Operator::eval(x, smem[item_ct1.get_local_id(2) + offset]);
    return x;
  }
};
...
struct Warp_reduce_linear
{
                                        ...
                                        sycl::nd_item<3> item_ct1)
  {
    ...
    for( int i = 1 ; i < NUM_STEPS ; ++i )
      if( my_lane_id < NUM_THREADS_PER_ITEM )
        smem[item_ct1.get_local_id(2)] = x = Operator::eval(
            x, smem[item_ct1.get_local_id(2) + i * NUM_THREADS_PER_ITEM]);
    return x;
  }
};
...
template <int NUM_THREADS_PER_ITEM, typename Operator, typename Value_type>
static __dpct_inline__ Value_type warp_reduce(volatile Value_type *smem,
                                              Value_type x)
{
